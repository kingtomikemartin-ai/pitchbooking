...
export function useBookings(selectedDate?: Date) {
  ...
  const bookingsWithParticipants: BookingWithParticipants[] = (bookingsQuery.data || []).map(booking => ({
    ...booking,
  ...
  const joinSession = useMutation({
    mutationFn: async ({ bookingId, playerName, playerLevel }: { bookingId: string; playerName: string; playerLevel: string }) => {
      const { data, error } = await supabase
        .from('booking_participants')
        .insert({
